...
export const addUser = user => async dispatch => {
  try {
    ...
    if (response.ok) {
      dispatch(setAlert('Пользователь создан', 'success', 5000));
...
export const updateUser = (user, id) => async dispatch => {
  try {
    ...
    const newUser = data.user;
    if (response.ok) {
      dispatch(setAlert('Пользователь обновлён', 'success', 5000));
...
export const deleteUser = id => async dispatch => {
  try {
    ...
    const data = await response.json();
    if (response.ok) {
      dispatch(setAlert('Пользователь удалён', 'success', 5000));
