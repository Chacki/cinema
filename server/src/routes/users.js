...
router.post('/change_role_admin', async (req, res) => {
  const { username } = req.body;
  const role = req.body.role ? req.body.role : 'superadmin';
  if (!username) {
    res.status(400).send({ message: 'Укажите username в бади!' });
  }
  try {
    ...
    res.send({ msg: `Роль обновлена: ${result}` });
  } catch (e) {
    res.sendStatus(400);
  }
});
